  ...
  const fetchClaimDetails = async (claimId: number) => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from("claims_table").select("*").eq("claim_id", claimId).maybeSingle();
  ...
  return <div className="min-h-screen bg-background">
      <div className="container mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-3xl font-bold text-foreground">Jessica Analyst Dashboard</h1>
        </div>
        ...
        {loading ? <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          </div> : claimDetails ? <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <StatCard title="AI Fraud Score" value={claimDetails["evidence_ai-score"] || "N/A"} icon={TrendingUp} description="Risk assessment" />

            <StatCard title="Financial Delta" value={`$${Math.abs((claimDetails["Estimated Damage"] || 0) - (claimDetails["Approved Amount"] || 0)).toLocaleString()}`} icon={DollarSign} description="Estimated vs Approved" />

            <StatCard title="Days Since Report" value={calculateDaysSince(claimDetails["Date Reported"])} icon={Clock} description="Time elapsed" />
          </div> : <div className="text-center p-12 bg-muted/30 rounded-lg border border-border">
